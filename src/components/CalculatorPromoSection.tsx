...
interface CalculatorPromoSectionProps {
  onGoToCalculatorWithVolume: (volume: number) => void;
}

export const CalculatorPromoSection: React.FC<CalculatorPromoSectionProps> = ({ onGoToCalculatorWithVolume }) => {
  const [quickVolume, setQuickVolume] = useState<string>('50,000');

  const handleCalculate = (e: React.FormEvent) => {
    e.preventDefault();
    const cleanNum = parseInt(quickVolume.replace(/[^0-9]/g, ''), 10) || 50000;
    onGoToCalculatorWithVolume(cleanNum);
  };
